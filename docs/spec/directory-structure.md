...
| `web/src/features/chat` | **チャット機能の単一の縦割り領域**。会話 UI、`generative-ui/`（Copilot frontend tool・declarative）、`schemas/`（zod）、hooks、context、型。 |
...
- Generative UI（frontend tool・declarative renderer など）はすべて **`web/src/features/chat`** 配下に置く。別 feature ディレクトリや `/api/generative-ui` のような追加 route は置かない。
